...
Optional for schema apply (code path ready through manifest **014**; runtime apply stays closed until independent verification + exact owner approval):
...
| `MIGRATION_APPROVED_VERSION` | Exact next pending manifest NNN only (one-step; never skip; never all remaining). Product tip **014**; CP0 baseline **008**. |

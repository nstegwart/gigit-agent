/**
 * Deterministic authenticated-shell assertions (TS mirror of qa/e2e/lib/auth-assert.mjs).
 *
...
export async function assertAuthenticatedOwnerShell(
  page: Page,
  opts: { boardId: string; allowLogin?: boolean },
): Promise<AuthShellResult> {
  ...
  if (!opts.allowLogin) {
    if (pathname.startsWith('/login') || pathname.startsWith('/setup')) {
      throw new Error(`AUTH_ASSERT FAIL: protected capture landed on auth surface url=${url}`)
    }
  }
  ...
  const probe = await page.evaluate(() => {
    ...
    return { loginForm, setupCopy, brand, sidebar, navLabels, authTitle }
  })

  if (!opts.allowLogin && (probe.loginForm || probe.setupCopy)) {
    throw new Error(
      `AUTH_ASSERT FAIL: login/setup form present on protected surface url=${url}`,
    )
  }

  if (!opts.allowLogin) {
    if (!probe.sidebar) throw new Error(`AUTH_ASSERT FAIL: OWNER sidebar missing url=${url}`)
    if (!probe.brand) throw new Error(`AUTH_ASSERT FAIL: brand-name missing url=${url}`)
    if (pathname.startsWith('/b/') && !pathname.includes(`/b/${boardId}`)) {
      throw new Error(`AUTH_ASSERT FAIL: boardId mismatch expected=${boardId} url=${url}`)
    }
  }
...
/**
 * Pure classifier for contract self-tests.
 * Structural setup is `hasSetupUi`. Body "bootstrap" prose is not a fail signal.
 */
export function classifyAuthSurface(s: {
  ...
  try {
    ...
    return { ok: false, reason: `url_parse:${String((e as Error)?.message || e)}` }
  }
}

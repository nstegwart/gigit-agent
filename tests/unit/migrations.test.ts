...
/** Product manifest 000..014 (paritas staging-migrations / MIGRATION_MANIFEST). */
const ALL_VERSIONS = [
  '000',
  '001',
  '002',
...
describe('migration manifest ordering + checksums', () => {
  it('has strictly ordered 000..014 entries', () => {
    ...
    expect(MIGRATION_MANIFEST.map((m) => m.classification)).toEqual([
      ...
      'REVERSIBLE', // 014 cp0 sync backlog sources
    ])
  })
  ...
  it('009 rebuild_lineage SQL is additive IF NOT EXISTS only', () => {
    const entry = MIGRATION_MANIFEST.find((m) => m.version === '009')!
    expect(entry.filename).toBe('009_rebuild_lineage.sql')
    expect(entry.classification).toBe('REVERSIBLE')
    const sql = fs.readFileSync(path.join(cwd, entry.relativePath), 'utf8')
    expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS rebuild_lineage_records/)
    const stmts = splitSqlStatements(sql).map((s) => s.toLowerCase())
    expect(stmts.length).toBeGreaterThan(0)
    for (const s of stmts) {
      expect(s).not.toMatch(/\bdrop\s+table\b/)
    ...
    expect(MIGRATION_MANIFEST.filter((m) => m.version === '009')).toHaveLength(1)
  })

  it('010 product_features SQL is additive IF NOT EXISTS only', () => {
    const entry = MIGRATION_MANIFEST.find((m) => m.version === '010')!
    expect(entry.filename).toBe('010_product_features.sql')
    expect(entry.classification).toBe('REVERSIBLE')
    const sql = fs.readFileSync(path.join(cwd, entry.relativePath), 'utf8')
    expect(sql).toMatch(/CREATE TABLE IF NOT EXISTS product_features/)
    const stmts = splitSqlStatements(sql).map((s) => s.toLowerCase())
    expect(stmts.length).toBeGreaterThan(0)
    for (const s of stmts) {
      expect(s).not.toMatch(/\bdrop\s+table\b/)
  ...
  it('014 CP0 backlog sources SQL is additive CREATE only (no sink zeros)', () => {
    const entry = MIGRATION_MANIFEST.find((m) => m.version === '014')!
    expect(entry.filename).toBe('014_cp0_sync_backlog_sources.sql')
    expect(entry.classification).toBe('REVERSIBLE')
    const sql = fs.readFileSync(path.join(cwd, entry.relativePath), 'utf8')
    expect(sql).toMatch(/control_plane_sync_outbox/)
    expect(sql).toMatch(/control_plane_legacy_residuals/)
    expect(sql).toMatch(/PENDING/)
    expect(sql).toMatch(/UNREPLAYED/)
    ...
    expect(MIGRATION_MANIFEST.filter((m) => m.version === '014')).toHaveLength(1)
  })
...
describe('migration plan / dry-run / idempotent rerun', () => {
  it('plans APPLY for empty history on LOCAL', () => {
    ...
    expect(plan.orderedVersions).toEqual([...ALL_VERSIONS])
    ...
    expect(plan.items.filter((i) => i.version === '009')).toHaveLength(1)
    ...
    expect(plan.items.filter((i) => i.version === '014')).toHaveLength(1)
    expect(plan.items.find((i) => i.version === '014')!.action).toBe('APPLY')
  })
  ...
  it('apply via memory executor is ordered and idempotent on second run', async () => {
    ...
    expect(first.applied).toEqual([...ALL_VERSIONS])
    ...
    // 006..014 each appear exactly once in apply output + history
    for (const v of [
      '006',
      '007',
      '008',
  ...
  it('plan/dry-run/status/apply include 008..014; partial 000-007 then apply 008..014', async () => {
    const loaded = loadMigrationManifest(cwd)
    ...
    expect(loaded.filter((m) => m.version === '014')).toHaveLength(1)
    ...
    expect(m014.filename).toBe('014_cp0_sync_backlog_sources.sql')
    ...
    ).toEqual(['008', '009', '010', '011', '012', '013', '014'])
    ...
    expect(first.ok).toBe(true)
    expect(first.applied).toEqual(['008', '009', '010', '011', '012', '013', '014'])
    ...
    expect(exec.history.find((h) => h.version === '009')!.sha256).toBe(
      m009.sha256,
    )
    ...
    expect(exec.history.find((h) => h.version === '014')!.sha256).toBe(
      m014.sha256,
    )
    expect(
...
describe('lifecycle mapping guard wired into migration plan/apply (AC-LIFE-02)', () => {
  it('G0 identity rows are representable and approve plan + apply', async () => {
    ...
    expect(exec.history).toHaveLength(15)
  })
...
describe('migrate-runner.mjs teardown exit honesty', () => {
  ...
  it('offline plan still exits 0 with ordered 000..014 (load/main path intact)', () => {
    ...
    expect(ordered[ordered.length - 1]).toBe('014')
